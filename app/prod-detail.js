	...
	$(function() {
		...
		$(".gavUl li").click(function () {
			...
			if ( $(this).hasClass("active") ) {
				...
				if (matchProdId) {
					window.location.href = '/crossfire/prodDetail/' + matchProdId;
				} else {
					disableAddCart = true;
					$("#addCart").css("background-color", "gray");
					if (!selFlag) {
		...
		$("#addCart").click(function () {
			...
			$.ajax({
				type: 'post',
				dataType: 'json',
				url: '/crossfire/userapi/addCartProd',
				data: {
					pid: $("#pid").val(), 
		...
		$("#buyNowBtn").click(function () {
			...
			$.ajax({
				type: 'post',
				dataType: 'json',
				url: '/crossfire/userapi/buyNow',
				data: {
					pid: $("#pid").val(), 
					pcount: $("#pcs").find("input").val()
				}
			}).done(function (r) {
				window.location.href = "/crossfire/user/settlement/" + r.orderId;
			}).fail(function (jqXHR, textStatus) { 
				publicTip.showLoadingToast(false);
				publicTip.showTip(jqXHR.responseJSON);
			});
		...
		$("#collectionHref").click(function () {
			...
			$.ajax({
				type: 'post',
				dataType: 'json',
				url: '/crossfire/userapi/collection/act',
				data: {
					id: id, 
					pid: $("#pid").val()
				}
