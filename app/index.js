...
define(["jquery", "vue", "vue-resource", "publicTip", "swiper-4.1.6.min", "public"], function($, Vue, vueResource, publicTip, Swiper, public){
	...
	$(function() {
		...
		$(".weui-tabbar__item").click(function () {
			...
			if (currentId == 'my') {
				...
				$.ajax({
					type: 'post',
					dataType: 'json',
					url: '/crossfire/userapi/getLoginUserInfo'
				}).done(function (r) {
					...
					publicTip.showLoadingToast(false);
					jqXHR.responseJSON.loginSuccUrl = "/crossfire?actTabbar=my";
		...
		$('#signoutHref').on('click', function(){
			publicTip.showConfirm('Confirm to log out？', function(){
				window.location.href = "/crossfire/signout";
			});
		});
		...
		var gcVm = new Vue({
			...
			methods: {
				...
				search: function (isLoadMore, tipKey, ot) {
					...
					that.$resource('/crossfire/api/search/' + that.key + "/"+ that.limit + "/" + that.offset + "/" + that.ordertype).get().then(function (resp) {
						that.loading = false;
