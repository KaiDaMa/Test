define(["jquery"], function($){
	...
	var showError = function (resp) {
		resp.json().then(function (result) {
			...
			if (result.code == 'login:must_login') {
				var loginSuccUrl = result.loginSuccUrl || window.location.pathname || "/crossfire/";
				window.location.href = '/crossfire/login?loginSuccUrl=' + loginSuccUrl;
			} else {
	...
	var showTip = function (resp) {
		...
		if (resp.code == 'login:must_login') {
			var loginSuccUrl = resp.loginSuccUrl || window.location.pathname || "/crossfire/";
			window.location.href = '/crossfire/login?loginSuccUrl=' + loginSuccUrl;
		} else {
